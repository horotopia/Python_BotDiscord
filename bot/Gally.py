...
########################################################################
	...
	if any([message.content.startswith (item) for item in ['volonteeternelle','eternelle']]):
		embed=discord.Embed(title="Permet de survivre 1 fois à une attaque mortelle.\nSi le sort est lancé une seconde fois : supprime tous les effets negatifs (en début de tour et ce pendant toute la durée de volonte Éternelle)", url='', color=0xffffff)
...
##############################################################
	...
	if message.content.startswith('LdRecUp'):
...
############################################
	...
	if any([message.content == (item) for item in ['Trink','trink']]):
	...
	if any([message.content == (item) for item in ['trinket','Trinket','Attirail','attirail']]):
		embed = discord.Embed(title="Attirail", url="",color=0xfff99e)
		embed.set_author(name="")
		embed.set_thumbnail(url="")
	...
	if any([message.content ==(item) for item in ['trinketHP','TrinketHP','AttirailHP','attirailHP', 'TrinketPV','trinketPV','TrinketPv','TrinketPv']]):
...
#######################################################################
	...
	if message.content.startswith('gdgegderrgdrgre'):
		await message.channel.send("****")
	
	if any([message.content.startswith (item) for item in ['NONSENSE']]):
		embed=discord.Embed(title="", url="https://media.discordapp.net/attachments/684370413958332436/791358798081818634/Rudolph_large.png", color=0xffffff)
		embed.set_author(name="")
		embed.set_thumbnail(url="https://media.discordapp.net/attachments/684370413958332436/791358798081818634/Rudolph_large.png")
		embed.add_field(name="- Rudolph", value="\n\n**Oh les copains qu'est ce que vous faites là ? Vous m'avez cherché tout ce temps ? :scream:  Je voulais juste vous préparer une surprise car j'ai cassé par mégarde d'étoile du grand sapin de la fabrique ... Puisque que vous êtes là profitons ensemble de la soirée au clair de lune avec ce magnifique sapin de Noël en plein air !Je vous souhaites à tous d'excellentes fêtes de fin d'années et pleins de merveilleuses choses pour l'année à venir ! Gros bisous à tous !!!:smiling_face_with_3_hearts: :christmas_tree: **", inline=False)

		await message.channel.send(embed=embed)
	
	if any([message.content.startswith (item) for item in ['Boussole']]):
		embed=discord.Embed(title="", url="https://media.discordapp.net/attachments/684370413958332436/791361569584840724/boussole.png", color=0xffffff)
		embed.set_author(name="")
		embed.set_thumbnail(url="https://media.discordapp.net/attachments/684370413958332436/791361569584840724/boussole.png")
		embed.add_field(name="__- Instructions :__\n", value="\n\n**- Ultima - Frioppe\n- Feu : Nezha -Garuda + Odin + Radic - Medusine + Dartagnan\n- Silver, Anu SE eau et Anu SE bois\n- ce que la forêt spectrale est au lac Aria\n- Givri\n- 1921451920**", inline=False)

		await message.channel.send(embed=embed)

	if any([message.content ==(item) for item in ['NONSENSE']]):
		embed=discord.Embed(title="", url="https://media.discordapp.net/attachments/789164669927292950/791049767546781696/christmas_tree.jpg", color=0xfff99e)
		embed.set_image(url="https://media.discordapp.net/attachments/789164669927292950/791049767546781696/christmas_tree.jpg")
